import { app, BrowserWindow, ipcMain, Menu, nativeImage, powerSaveBlocker, Tray } from 'electron';
  ...
  const detailHtml = `
    ...
    <body>
      ...
      <div class="info-item">
        ...
        <div class="value">
          <span id="status-badge" class="status-badge status-connected">정상 동작 중</span>
        </div>
      </div>
      <div class="info-item">
...
function updateOverlayStatus(connected: boolean, message?: string) {
  if (overlayWindow && !overlayWindow.isDestroyed()) {
    overlayWindow.webContents.send('update-status', {
      connected,
      message: message || (connected ? '실행중' : '연결 끊김')
    });
  }
...
app.on('ready', () => {
  ...
  ipcMain.on('request-initial-status', () => {
    updateOverlayStatus(trayStatus === 'connected', trayStatus === 'connected' ? '실행중' : '연결 끊김');
  });
...
setInterval(() => {
  ...
  if (delay > PING_TIMEOUT_MS) {
    if (trayStatus === 'connected') {
      ...
      updateOverlayStatus(false, '연결 끊김');
    }
  } else {
