...
\*\*테스트 필요 항목
RegistServerCallback을 실패하고 있는 상황에서 Command를 통한 통신이 이루어지고 나서야 RegistServerCallback가 성공하는지 테스트 필요
만약 그럴 경우 request connkey를 agent가 직접 하도록 변경 필요

- agent tray가 검은색이면?
  okpos dll에 ping 날리고 실패 또는 응답이 없는 경우에 해당
  -1001 코드가 올경우 okpos 측에 상점 등록이 되었는지 먼저 확인 필요
...
- version log
  ...
  1.2.0 : 플로팅 오버레이 화면을 추가하여 사용자가 Agent가 켜져있는지 여부를 바로 확인할 수 있음
  1.2.1 : 플로팅 오버레이가 표시된 상태에서 OKPOS 종료 버튼 등 하단 UI가 클릭되지 않는 버그 수정 (오버레이 투명 영역의 마우스 이벤트 차단 문제 해결 - setIgnoreMouseEvents 적용)
  1.3.0 : nsis로 빌드형식 변경, 시작프로그램 자동등록, 자동업데이트 기능, 키 설정 탭 추가
